...
Public Function showImpostazioni(titolo As String, note As String, titoloAsseX As String, titoloAsseY As String, tipoGrafico As MSChart20Lib.VtChChartType, indicatori As Boolean, legenda As Boolean, serieUnite As Boolean, AnniConfronto As String, AnnoDiRiferimento As Integer, sCodIstat As String)
    ...
    Set rs = gDBC.Execute("SELECT DISTINCT AnaComune.AnnoRiferimento, s.Descrizione " & _
                          " FROM AnagraficaComunale AS AnaComune LEFT JOIN simulazioni AS s  ON (AnaComune.AnnoRiferimento = s.idSimulazione) AND (AnaComune.CodComune = s.codComune)" & _
                          " where AnaComune.CodComune='" & sCodIstat & "' order by AnaComune.AnnoRiferimento ")
