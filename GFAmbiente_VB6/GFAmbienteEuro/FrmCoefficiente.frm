...
Public Sub Carica(Optional sCodComune As String = vbNullString, Optional AnnoRiferimento As String = vbNullString, Optional tipoCalcolo As tpoTariffa = TariffaNonImpostata, Optional modal As Boolean = True)
    ...
    If GetParamValue(mAnnoRiferimento, mCodComune, "TIPOMETODO") = Tariffa507 Then
        tabStrip.Tabs("507UA").Selected = True
    End If
...
Private Sub DataGrid1_AfterColUpdate(index As Integer, ByVal ColIndex As Integer)
    Dim i As Integer
    If (Round(DataGrid1(index).Columns(ColIndex).value, 4)) <> DataGrid1(index).Columns(ColIndex).value Then
       DataGrid1(index).Columns(ColIndex).value = Round(DataGrid1(index).Columns(ColIndex).value, 4)
       Select Case mTipoCoeff
            Case tpo_coeff_507UA, tpo_coeff_507UD
                Dim bookmarker As Variant
                bookmarker = DataGrid1(index).Bookmark
                If DataGrid1(index).Columns("IPS CALC").value <> 0 Then
                    ADO1.Recordset.MoveFirst
                    Do While Not ADO1.Recordset.EOF
                        On Error Resume Next
                        ADO1.Recordset.fields("ValoreCalc").value = 0
                        ADO1.Recordset.fields("IPSCalc").value = 0
                        ADO1.Recordset.MoveNext
                    Loop
                End If
                DataGrid1(index).Bookmark = bookmarker
        End Select
    End If
End Sub
...
'        MsgBox "Attenzione non sono stati trovati i dati anagrafici", vbExclamation + vbOKOnly, App.Title
    ...
    If mTipoCoeff = tpo_coeff_507UA Then
        ...
        ADO1.RecordSource = "SELECT ValCoeff507CA.AnnoRiferimento,ValCoeff507CA.CodAttivita,ValCoeff507CA.CodComune, ValCoeff507CA.IQS,ValCoeff507CA.IPS,ValCoeff507CA.IPSCALC, CategorieAttivitaComunali.Descrizione " & _
                             "FROM  CategorieAttivitaComunali INNER JOIN ValoriCoefficientiIQSCAComunali as ValCoeff507CA ON ValCoeff507CA.CodAttivita = CategorieAttivitaComunali.IdCategoriaAttivita and " & _
                             " ValCoeff507CA.CodComune = CategorieAttivitaComunali.CodComune and " & _
                             " ValCoeff507CA.AnnoRiferimento = CategorieAttivitaComunali.AnnoRiferimento " & _
                             "WHERE ValCoeff507CA.CodComune='" & mCodComune & "'and ValCoeff507CA.AnnoRiferimento='" & mAnnoRiferimento & "' order by ValCoeff507CA.CodAttivita"
    Else
        ADO1.RecordSource = "SELECT ValCoeff507UD.CodComune,NumeroComponentiFamiglia.Descrizione, ValCoeff507UD.AnnoRiferimento, ValCoeff507UD.CodUtenza, ValCoeff507UD.Valore, ValCoeff507UD.ValoreCalc  " & _
                            "FROM NumeroComponentiFamiglia INNER JOIN (UtenzeDomesticheComunali INNER JOIN ValoriCoefficienti507UDComunali as ValCoeff507UD ON (UtenzeDomesticheComunali.IdNumeriComponenti = ValCoeff507UD.CodUtenza) AND (UtenzeDomesticheComunali.AnnoRiferimento = ValCoeff507UD.AnnoRiferimento) AND (UtenzeDomesticheComunali.CodComune = ValCoeff507UD.CodComune)) ON NumeroComponentiFamiglia.NumeroComponenti = UtenzeDomesticheComunali.IdNumeriComponenti " & _
                            "WHERE (((ValCoeff507UD.CodComune)='" & mCodComune & "') AND ((ValCoeff507UD.AnnoRiferimento)='" & mAnnoRiferimento & "')) order by ValCoeff507UD.CodUtenza"
    End If
    ADO1.Refresh
   ...
   Txt(COEFF_IQSUD_ID) = Format$(GetParamValue(mAnnoRiferimento, mCodComune, "COEFFIQSUD"), FORMAT4DEC)
   Txt(COEFF_IPSUD_ID) = Format$(GetParamValue(mAnnoRiferimento, mCodComune, "COEFFIPSUD"), FORMAT4DEC)
...
Sub CalcolaIPS()
    ...
    KM = rsDatiTecniciComune("QRifiutiTotali") / (rsDatiTecniciComune("TotSuperficieIscrRuolo") - (TotSuperficieInRiduzioniDAUA_PF + TotSuperficieInRiduzioniDAUD_PF))
    ...
    If mTipoCoeff = tpo_coeff_507UD Then
        ...
        Txt(COEFF_IPSUD_ID).Text = Format$(IPSUD, FORMAT4DEC)
    Else
        'KD
