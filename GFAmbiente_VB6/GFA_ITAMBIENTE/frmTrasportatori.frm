...
Begin VB.Form frmTrasportatori 
   Caption         =   "Elenco Trasportatori"
   ClientHeight    =   8610
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   13245
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin MSDataListLib.DataCombo ctlComune 
         Bindings        =   "frmTrasportatori.frx":0000
         DataField       =   "CODISTAT"
         Height          =   360
         Left            =   3360
         TabIndex        =   4
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin MSDataGridLib.DataGrid DataGrid1 
         Bindings        =   "frmTrasportatori.frx":0024
         Height          =   2415
         Left            =   240
         TabIndex        =   24
         Top             =   480
   ...
   End
End
Attribute VB_Name = "frmTrasportatori"
...
Const CMD_CHIUDI = 0
Const CMD_AUTOMEZZI = 1
Const CMD_AUTORIZZAZIONI = 2
Const CMD_CONDUCENTI = 3
Const CMD_RICERCA = 4
Const CMD_SALVA = 5
Const CMD_NUOVO = 6
Const CMD_CANCELLA = 7
    ...
    Select Case Index
        ...
        Case CMD_CONDUCENTI
            If DataGrid1.Row <> -1 Then
                ...
                frmConducenti.showMe DataGrid1.Columns("Codice").Value, Des, "T"
            End If
        Case CMD_AUTOMEZZI
            If DataGrid1.Row <> -1 Then
                Des = nullToBlank(DataGrid1.Columns("Ragione Sociale").Value)
                frmAutomezzi.showMe DataGrid1.Columns("Codice").Value, Des, "T"
            End If
        ...
        Case CMD_SALVA
            ...
            pErrore = DevEnvMDB.rscmdTrasportatori.EditMode <> adEditNone
            If DevEnvMDB.rscmdTrasportatori.EditMode <> adEditNone Then
                DevEnvMDB.rscmdTrasportatori.UpdateBatch adAffectAllChapters
                DataGrid1.Refresh
                
                cmd(CMD_AUTOMEZZI).enabled = True
                cmd(CMD_AUTORIZZAZIONI).enabled = True
                cmd(CMD_CONDUCENTI).enabled = True
        ...
        Case CMD_NUOVO
            ...
            DevEnvMDB.rscmdTrasportatori.AddNew
            
            cmd(CMD_AUTOMEZZI).enabled = False
            cmd(CMD_AUTORIZZAZIONI).enabled = False
            cmd(CMD_CONDUCENTI).enabled = False
            
        Case CMD_CANCELLA
            If (MsgBox("Attenzione, se sicuro di voler cancellare il trasportatore:" & DevEnvMDB.rscmdTrasportatori("RagioneSociale"), vbQuestion + vbYesNo) = vbYes) Then
                'cancella autorizzazioni
                gDBC.Execute "delete * from Autorizzazioni where idsoggetto=" & DevEnvMDB.rscmdTrasportatori("idTrasportatore") & " and codSoggetto='T'"
                'cancella automezzi
                gDBC.Execute "delete * from Automezzi where codUtente=" & DevEnvMDB.rscmdTrasportatori("idTrasportatore") & " and codSoggetto='T'"
                'cancella Conducenti
                gDBC.Execute "delete * from Conducenti where idTrasportatore=" & DevEnvMDB.rscmdTrasportatori("idTrasportatore")

                
                DevEnvMDB.rscmdTrasportatori.Delete
                DevEnvMDB.rscmdTrasportatori.UpdateBatch adAffectAllChapters
            End If
        Case CMD_AUTORIZZAZIONI
            If DataGrid1.Row <> -1 Then
                Des = nullToBlank(DataGrid1.Columns("Ragione Sociale").Value)
                frmAutorizzazioni.showMe DataGrid1.Columns("codice").Value, "T", Des
            End If
    End Select
End Sub
...
Private Sub ricaricaLista()
    ...
    DevEnvMDB.rscmdTrasportatori.Filter = search
    If search = "" Then
        DevEnvMDB.rscmdTrasportatori.Filter = search & " RagioneSociale <> '123'"
    End If
...
Private Sub DataGrid1_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
    If DataGrid1.Row >= 0 Then ctlComune.Text = nullToBlank(DevEnvMDB.rscmdTrasportatori.Fields("comune"))
End Sub
...
Private Sub Form_Load()
    ...
    sourceElenco = DevEnvMDB.rscmdTrasportatori.Source
    ...
    Exit Sub
LoadERR:
    MsgBox "Load frmTrasoprtatori:" & Err.Description, vbCritical, App.Title
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    If DevEnvMDB.rscmdTrasportatori.state = adStateOpen Then
        DevEnvMDB.rscmdTrasportatori.Close
    End If
    
    DevEnvMDB.rscmdTrasportatori.Filter = vbNullString
...
Private Sub cancelCurrentOperation()
   On Error Resume Next
    If DevEnvMDB.rscmdTrasportatori.EditMode = adEditAdd Then
        DevEnvMDB.rscmdTrasportatori.CancelBatch
                If (Not DevEnvMDB.rscmdTrasportatori.EOF And Not DevEnvMDB.rscmdTrasportatori.BOF) Then
            DevEnvMDB.rscmdTrasportatori.MoveFirst
        End If
    ElseIf DevEnvMDB.rscmdTrasportatori.EditMode = adEditInProgress Then
        DevEnvMDB.rscmdTrasportatori.CancelUpdate
    End If
End Sub
